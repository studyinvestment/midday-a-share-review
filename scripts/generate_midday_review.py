...
def build_s01_note():
    ...
    if br.get("missing"):
        ...
        # 2026-09-17 修复缺陷 #46：原 neutral 分支（40% ≤ up_ratio < 55%）**一档到底**，
        # 无条件写「指数与个股方向一致、涨跌大致相当，行情结构偏均衡，主线与补涨并存」。
        # 但全库其余 9 处广度判定一律在 **45** 分档（L172/208/705/837/840/845/848/851/890），
        # 唯独此处把 40–45% 这一段也算进"均衡"。实测 2026-09-17 up_ratio=40.7%
        # （2260 涨 / 3102 跌）已贴弱档下沿，本句与同页 M06「指数与个股同步走弱，
        # 缺少逆势赚钱效应」、M08「上涨由少数主线拉动」直接互斥
        # （属 #34 同族：跨模块关系断言未按广度分档细分）。
        # → 在 neutral 内以 45 再分两档，并与指数方向（idx_dir 单一事实源）四态组合。
        _lead = ("指数内部分化" if _split else
                 {"up": "指数维持在正收益区间", "down": "指数普遍收跌",
                  "flat": "指数大体走平"}.get(_trend, "指数方向中性"))
        if up_ratio < 45:
            s += (f"{_lead}；但从个股看<b>下跌家数多于上涨</b>——仅 {BR_up} 只上涨"
                  f"（{up_ratio:.1f}%，不足五成）、{BR_dn} 只下跌，<b>结构偏弱</b>，"
                  "上涨集中于少数主线，普涨面有限，不宜按均衡市处理（详见第 4 节涨跌家数）。")
        else:
            s += (f"{_lead}；个股层面涨跌大致相当——{BR_up} 只上涨（{up_ratio:.1f}%）、"
                  f"{BR_dn} 只下跌，行情结构偏均衡，主线与补涨并存（详见第 4 节涨跌家数）。")
    elif _split:
...
def build_s02():
    ...
    bounce = cp - lo
    blk = []
    # 2026-09-17 修复缺陷 #45：开盘段原为「低开 → 无条件写『开盘后短暂下探至上午最低 X』」，
    # **未校验低点出现时刻**。实测 2026-09-17：上证低开 -0.38%，上午低点 3868.10（-0.60%）
    # 迟至 10:14 才出现（09:34 反而先创出 +0.18% 的上午高点）——该句既把 10:14 的低点
    # 错记成开盘时段，又与同一模块紧随其后的「早盘 09:34 冲高 / 盘中 10:14 回落」直接矛盾
    # （属 #39 同族：时段动词未接实际时刻）。同族问题在**高开侧对称存在**：
    # 「开盘后短暂冲高至上午最高」在最高点出现于盘中时同样不成立。
    # → 改为按极值实际出现时刻判定：仅当极值确实落在开盘段（09:30–09:40）内才用"开盘后短暂…"，
    #   否则如实写出极值出现时刻，并显式声明"并非开盘时段形成"。
    OPEN_END = "0940"                      # 与开盘段标题 09:30–09:40 对齐
    _hi_early = (hi == op) or (hit <= OPEN_END)
    _lo_early = (lo == op) or (lot <= OPEN_END)
    if hi == op:
        _open_txt = f"开盘即为上午最高点 {m['high']:.2f}（{hi:+.2f}%），其后单边下行。"
    elif op < 0:
        if lo == op:
            _open_txt = f"开盘即为上午最低 {m['low']:.2f}（{lo:+.2f}%），此后未再跌破该位置。"
        elif _lo_early:
            _open_txt = f"开盘后短暂下探至上午最低 {m['low']:.2f}（{lo:+.2f}%）。"
        else:
            _open_txt = (f"开盘后弱势震荡，<b>上午低点迟至 {lot[:2]}:{lot[2:]} 才出现</b>"
                         f"（{m['low']:.2f}，{lo:+.2f}%），并非开盘时段形成。")
    else:
        if _hi_early:
            _open_txt = f"开盘后短暂冲高至 {m['high']:.2f}（{hi:+.2f}%）。"
        else:
            _open_txt = (f"开盘后高位震荡，<b>上午高点迟至 {hit[:2]}:{hit[2:]} 才出现</b>"
                         f"（{m['high']:.2f}，{hi:+.2f}%），并非开盘时段形成。")
...
MCP_NOTE = (args.mcp_note.strip() if args.mcp_note else
            ...
            # 2026-09-17 补：默认页脚原先只声明"用了什么源"，未声明"没做什么校验"。
            # 实测同一份数据、同一渲染器，**带 --mcp-note 与不带**产出的页脚差异明显
            # （前者含交叉校验结论与精度下降项，后者二者皆无）——「效果取决于调用方有没有记得写」。
            # 精度下降项应**默认可见**：它是读报告的人判断可信区间的依据，不该靠人肉补。
            "本期<b>未做连接器侧逐项交叉校验</b>；精度下降项：① 板块主力资金流仅当日口径，"
            "缺 5 日 / 20 日维度；② 行业与概念板块未做第二数据源比对。"
            "接入 MCP 连接器后可用 --mcp-note 覆盖本段，写入逐项交叉校验结论。")
